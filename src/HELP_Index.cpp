#include <eye/index_graph.h>
#include <eye/exceptions.h>
#include <eye/parameters.h>
#include <omp.h>
#include <set>
#include <queue>
#include <stack>
      ...
      graph_[n].join([&](unsigned i, unsigned j)
                     {
                       if (i != j)
                       {
                         ...
                         for (int k = 0; k < attribute_number_; k++)
                         {
                           ...
                           {
                             cnt+=abs(attributes_[i][k] - attributes_[j][k]);
                           }
                         }
    ...
    for (unsigned i = 0; i < graph_[q].pool.size() && i < bK; i++)
    {
      ...
      for (unsigned nn = 0; nn < graph_[nid].pool.size() && i < bK; nn++)
      {
        ...
        for (int k = 0; k < attribute_number_; k++)
        {
          ...
          {
            cnt+=abs(attributes_[q][k] - attributes_[nnid][k]);
          }
        }
        fusion_distance(dist, cnt);
    ...
    while (result.size() < range && (++start) < pool.size())
    {
      ...
      for (unsigned t = 0; t < result.size(); t++)
      {
        ...
        for (int k = 0; k < attribute_number_; k++)
        {
          ...
          {
            cnt+=abs(attributes_[result[t].id][k]-attributes_[p.id][k]);
          }
        }
    ...
    for (unsigned i = 0; i < c.size(); i++)
    {
      ...
      for (unsigned j = 0; j < N; j++)
      {
        ...
        for (int k = 0; k < attribute_number_; k++)
        {
          ...
          {
            cnt+=abs(attributes_[c[i]][k]- attributes_[j][k]);
          }
        }
        if (cnt > 0) continue;
        //fusion_distance(dist, cnt);
    ...
    for (unsigned i = 0; i < nd_; i++)
    {
      ...
      for (unsigned j = 0; j < S; j++)
      {
        ...
        for (int k = 0; k < attribute_number_; k++)
        {
          ...
          {
            cnt+=abs(attributes_[i][k] - attributes_[id][k]);
          }
        }
        fusion_distance(dist, cnt);
    ...
    for (unsigned i = 0; i < nd_; i++)
    {
      ...
      for (unsigned j = 0; j < K; j++)
      {
        ...
        for (int k = 0; k < attribute_number_; k++)
        {
          if (attributes_[i][k] != attributes_[id][k])
          {
            cnt+= (attributes_[i][k] - attributes_[id][k]);
          }
        }
        fusion_distance(dist, cnt);
...
#pragma omp parallel for
    for (unsigned i = 0; i < control_points.size(); i++)
    {
        ...
        for (unsigned j = 0; j < nd_; j++)
        {
            float dist = distance_->compare(data_ + control_points[i] * dimension_, data_ + j * dimension_, dimension_);

            float cnt = 0;
            for (int k = 0; k < attribute_number_; k++)
            {
                if (attributes_[control_points[i]][k] != attributes_[j][k])
                {
                    cnt+= abs(attributes_[control_points[i]][k] - attributes_[j][k]);
                }
            }
            if (cnt > 0) continue;
            //fusion_distance(dist, cnt);
        ...
        {
            acc_eval_set[i].push_back(tmp[j].id);
        }
    }
...
#pragma omp parallel for
    ...
    {
        float acc = 0;
        auto& g = final_graph_[control_points[i]];
        auto& v = acc_eval_set[i];
        for (unsigned j = 0; j < g.size() && j < 10; j++)
        {
        ...
        mean_acc += acc / (g.size() < 10 ? float(g.size()) : 10.0);
    }
    float ret_recall = mean_acc / control_points.size();
    std::cout << "Graph Quality : " << ret_recall << std::endl;
    has_built = true;
  }
      ...
      unsigned *indices)
  {
    ...
    for (unsigned i = 0; i < L; i++)
    {
      ...
      for (int k = 0; k < attribute_number_; k++)
      {
        ...
        {
          cnt+=abs(attributes_[query_id][k] - attributes_[id][k]);
        }
      }
      fusion_distance(dist, cnt);
  ...
  void IndexGraph::Save(const char *filename)
  {
    ...
    for (unsigned i = 0; i < nd_; i++)
    {
      out.write((char *)attributes_[i].data(), attribute_number_ * sizeof(int));
    }
    out.close();
  }
  ...
  void IndexGraph::Load(const char *filename)
  {
    ...
    while (!in.eof())
    {
      std::vector<int> tmp(attribute_number_);
      in.read((char *)tmp.data(), attribute_number_ * sizeof(int));
      if (in.eof())
        break;
  ...
  void IndexGraph::SearchWithOptGraph(std::vector<int> attribute,
                                      ...
                                      unsigned *indices)
  {
    ...
    for (unsigned i = 0; i < init_ids.size(); i++)
    {
      ...
      float cnt = 0;
      int*id_attribute = (int*)(opt_graph_ + node_size * id + data_len);
      for (int k = 0; k < attribute_number_; k++)
      {
        if (id_attribute[k] != attribute[k])
        {
          cnt+=abs((id_attribute[k] - attribute[k]));
        }
      }
    ...
    while (k < (int)L)
    {
      ...
      if (retset[k].flag)
      {
        ...
        for (unsigned m = 0; m < MaxM; ++m)
        {
          ...
          //dist += dist * cnt / (float)attribute_number_;
          dist += cnt * weight_search;
  ...
  void IndexGraph::SearchWithOptGraph(std::vector<std::string> attributes,
                                      ...
                                      unsigned *indices)
  {
    std::vector<int> attribute = Attribute2int(attributes);
    ...
    while (k < (int)L_)
    {
      ...
      if (retset[k].flag)
      {
        ...
        for (unsigned m = 0; m < MaxM; ++m)
        {
          //unsigned id = neighbors[random_ids[m]];
            unsigned id = neighbors[m];
          if (flags[id])
            continue;
          flags[id] = 1;
          ...
          dist_cout++;
          if (dist >= retset[L_ - 1].distance)
            continue;
          Neighbor nn(id, dist, true);
          int r = InsertIntoPool(retset.data(), L_, nn);
          count++;
          // if(L+1 < retset.size()) ++L;
          if (r < nk)
            nk = r;
  ...
  { // use after build or load
    ...
    attribute_len = attribute_number_ * sizeof(int);
    ...
    for (unsigned i = 0; i < nd_; i++)
    {
      ...
      std::memcpy(cur_node_offset + 2 * sizeof(unsigned), final_graph_[i].data(),
                  k * sizeof(unsigned));
      std::vector<int>().swap(attributes_[i]);
      std::vector<unsigned>().swap(final_graph_[i]);
    }
    //free(data);
    data_ = nullptr;
    std::vector<std::vector<int>>().swap(attributes_);
    CompactGraph().swap(final_graph_);
  }
...
#pragma omp for
  ...
  std::vector<int> IndexGraph::Attribute2int(std::vector<std::string> str)
  {
    std::vector<int> tmp;
  ...
  void IndexGraph::AddAllNodeAttributes(std::vector<std::string> attributes)
  {
    ...
    if (attributes_kindnum.size() != attribute_number_)
    {
        attributes_kindnum.resize(attribute_number_);
    }
    ...
    for (int i = 0; i < attributes.size(); i++)
    {
      ...
      if (std::count(attributes_kindnum[i].begin(), attributes_kindnum[i].end(), attributes[i]) == 0) {
          attributes_kindnum[i].push_back(attributes[i]);
      }
      if (flag)
      {
  ...
  void IndexGraph::setAbility(const Parameters& parameters) {
      alpha = parameters.Get<float>("T");
      for (int j = 0; j < attributes_kindnum.size(); j++) {
          ab *= attributes_kindnum[j].size();
      }
