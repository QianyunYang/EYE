...
int main(int argc, char **argv)
{
  if (argc != 15)
  {
    std::cout << argv[0] << " data_file att_file save_graph save_attributetable K L iter S R Range PL B M T"
              << std::endl;
    exit(-1);
  }
  ...
  data_load = eye::data_align(data_load, points_num, dim); //one must align the data before build
  ...
  eye::IndexGraph index(dim, points_num, eye::L2, (eye::Index *)(&init_index));
  ...
  paras.Set<float>("T", atof(argv[14]));
  index.setAbility(paras);
