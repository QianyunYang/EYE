...
int main(int argc, char **argv)
{
  if (argc != 10)
  {
    std::cout << argv[0] << " graph_path attributetable_path data_path query_path query_att_path groundtruth_path L_search weight_search recall@k"
              << std::endl;
    exit(-1);
  }
  ...
  eye::load_data(data_path, data_load, points_num, dim);
  data_load = eye::data_align(data_load, points_num, dim);
  char *query_path = argv[4];
  //std::cerr << "Query Path: " << query_path << std::endl;
  unsigned query_num, query_dim;
  float *query_load = nullptr;
  eye::load_data(query_path, query_load, query_num, query_dim);
  query_load = eye::data_align(query_load, query_num, query_dim);
  ...
  eye::IndexRandom init_index(dim, points_num);
  eye::IndexGraph index(dim, points_num, eye::FAST_L2,
                             (eye::Index *)(&init_index));
  ...
  unsigned search_k =  atoi(argv[9]);
  float weight_search = 140000;
  eye::Parameters paras;
  unsigned L_search;
  std::sscanf(argv[7], "%d", &L_search);
  paras.Set<unsigned>("L_search", L_search);
  paras.Set<float>("weight_search", atof(argv[8]));
  ...
  float latency_stats = (float)(diff.count() * 1000000) / static_cast<float>(query_num);
  int cnt = 0;
  for (unsigned i = 0; i < ground_num; i++)
  {
    for (unsigned j = 0; j < search_k; j++)
    {
      ...
      for (; k < search_k; k++)
      {
        if (res[i][k] == ground_load[i * ground_dim + j])
          break;
      }
      if (k != search_k)
        cnt++;
  ...
  std::cout << "Search Time: " << diff.count() << "QPS:" << ground_num * 1.0 / (1.0 * diff.count()) << " " << search_k << " Recall@ "<<search_k<<": " << acc << "Mean Latency: " << latency_stats << std::endl;
